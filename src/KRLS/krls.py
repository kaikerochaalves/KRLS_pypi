...
class SW_KRLS(base):
    ...
    def __init__(self, N = 30, c = 0.1, validate_array = False, kernel_type = 'Gaussian', **kwargs):
...
class EX_KRLS(base):
    ...
    def __init__(self, alpha = 0.999, beta = 0.995, c = 1E-2, q = 1E-3, N = 30, validate_array = False, kernel_type = 'Gaussian', **kwargs):
        ...
        c : float, default=1E-2
            Regularization parameter. Must be a very small float.

        q : float, default=1E-3
...
class KRLS_T(base):
    ...
    def __init__(self, N = 30, c = 0.1, gamma = 0.999 , sn2 = 1e-2, validate_array = False, kernel_type = 'Gaussian', **kwargs):
    ...
    def KRLS(self, x, y):
        ...
        self.parameters_dict["Theta"] = np.sqrt( self.gamma ) * self.parameters_dict["Theta"]
...
class QKRLS(base):
    ...
    def __init__(self, N = 1000, c = 0.1, epsilon = 0.01, validate_array = False, kernel_type = 'Gaussian', **kwargs):
    ...
    def KRLS(self, x, y):
        ...
        if distance[j] <= self.epsilon:
            ...
            # Update Kinv
            self.parameters_dict["Kinv"] = self.parameters_dict["Kinv"] - ( np.outer(Kinvj, np.dot(Kj, self.parameters_dict["Kinv"])) ) / ( 1 + np.dot(Kj, Kinvj) )
            # Updating Theta
            self.parameters_dict["Theta"] = self.parameters_dict["Theta"] + np.dot(Kinvj, ( y - np.dot(Kj, self.parameters_dict["Theta"]) ) ) / ( ( 1 + np.dot(Kj, Kinvj) ) )
            ...
            self.parameters_dict["Kinv"][:-1,-1] = - z_Lambda
            self.parameters_dict["Kinv"][-1,:-1] = - z_Lambda
            self.parameters_dict["Kinv"] = ( 1 / r ) * self.parameters_dict["Kinv"]
            # Update Theta
            self.parameters_dict["Theta"] = self.parameters_dict["Theta"] - ( z_Lambda * ( 1 / r ) * e )
...
class ANS_KRLS(base):
    ...
    def __init__(self, N = 1000, nu = 0.01, c = 1e-6, epsilon = 0.01, mu = 1, validate_array = False, kernel_type = 'Gaussian', **kwargs):
        ...
        mu : integer, default=1
            Coherence coefficient threshold. Must be an integer greater than 0.
    ...
    def KRLS(self, x, y):
        ...
        EstimatedError = ( y - np.matmul(k_til.T, self.parameters_dict["Theta"]) )
        
        # Novelty criterion
        if delta > self.nu and mu.item() <= self.Nu and distance[j] > self.epsilon and self.parameters_dict["Dict"].shape[0] < self.N:
        ...
        elif ( delta <= self.nu or mu.item() > self.Nu ) and distance[j] <= self.epsilon:
...
class FD_KRLS(base):
    ...
    def __init__(self, N = 200, c = 0.01, validate_array = False, kernel_type = 'Gaussian', **kwargs):
        
        """
        Fixed Dictionary Kernel Recursive Least Squares (FD-KRLS) model.
